  ...
  useEffect(() => {
    const fetchApplication = async () => {
      try {
        ...
        // Fetch user profile
        const profileResponse = await fetch(`/api/profile/${data.application.user.id}`)
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button className="mt-4" onClick={() => router.push("/admin/applications")}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <Button variant="ghost" className="mb-2 p-0 text-muted-foreground" onClick={() => router.back()}>
            ← Back
          </Button>
          <h2 className="text-2xl font-bold tracking-tight">Application for {application.job.title}</h2>
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Information extracted from the applicant&apos;s CV</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <p className="text-muted-foreground">{profile?.experience || "No experience information available"}</p>
              </div>
              <div>
                <h3 className="text-lg font-medium mb-2">Education</h3>
                <p className="text-muted-foreground">{profile?.education || "No education information available"}</p>
              </div>
            </CardContent>
          </Card>
        </div>
